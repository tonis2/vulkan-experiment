...
pub const PngError = error{ InvalidHeader, InvalidFilter, UnsupportedFormat };
...
const ColorType = enum(u8) { Greyscale = 0, Truecolor = 2, IndexedColor = 3, GreyscaleAlpha = 4, TruecolorAlpha = 6 };
...
const IHDR = struct { width: u32, height: u32, bitDepth: u8, colorType: ColorType, compressionMethod: CompressionMethod, filterMethod: u8, interlaceMethod: u8 };

inline fn filterNone(image: []const u8, line: []u8, y: u32, start: usize, bytes: u8) void {
    // line is already pre-filled with original data, so nothing to do
}

inline fn filterSub(image: []const u8, line: []u8, y: u32, start: usize, bytes: u8) void {
    var pos: usize = bytes;
...
inline fn filterUp(image: []const u8, line: []u8, y: u32, start: usize, bytes: u8) void {
...
inline fn filterAverage(image: []const u8, line: []u8, y: u32, start: usize, bytes: u8) void {
    const width = line.len;
    var pos: usize = 0;
    while (pos < line.len) : (pos += 1) {
...
inline fn filterPaeth(image: []const u8, line: []u8, y: u32, start: usize, bytes: u8) void {
    const width = line.len;
    var pos: usize = 0;
    while (pos < line.len) : (pos += 1) {
...
fn readChunk(allocator: *Allocator, reader: anytype) !Chunk {
    ...
    var stream = ChunkStream{ .buffer = data, .pos = 0 };
...
pub fn read(allocator: *Allocator, unbufferedReader: anytype) !Image {
    ...
    var idatBuffer = (std.io.BufferedReader(64 * 1024, @TypeOf(zlibReader)){ .unbuffered_reader = zlibReader });
    ...
    const Filter = fn (image: []const u8, line: []u8, y: u32, start: usize, bytes: u8) void;
    ...
    if (ihdr.colorType == .Truecolor) {
        ...
        return Image{ .allocator = allocator, .data = imageData, .width = ihdr.width, .height = ihdr.height, .format = @import("image.zig").ImageFormat.RGB24 };
        ...
        return Image{ .allocator = allocator, .data = imageData, .width = ihdr.width, .height = ihdr.height, .format = @import("image.zig").ImageFormat.RGBA32 };
    } else {
